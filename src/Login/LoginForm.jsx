...
export default function LoginForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // const response = await axios.post('https://ventures-designed-seas-hang.trycloudflare.com/api/auth/login', {
            const response = await axios.post('http://localhost:8083/api/auth/login', {
                email: loginData.email,
                password: loginData.password,
            }, {
                withCredentials: true  // Penting untuk otentikasi berbasis cookie
            });

            // Backend menggunakan cookie, tapi respons mungkin juga berisi token JWT
            console.log('Login response:', response.data);

            // Cek apakah respons berisi token JWT dan simpan di localStorage
            // Asumsi: Backend mungkin mengembalikan token di properti 'token', 'jwt', atau 'accessToken'
            ...
            if (token) {
                ...
                // Anda bisa menambahkan penanganan error atau pesan kepada pengguna di sini
                // jika token dianggap wajib meskipun backend juga menggunakan cookie.
            }
